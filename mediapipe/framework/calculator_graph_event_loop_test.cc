...
TEST_F(CalculatorGraphEventLoopTest, FailingEventLoop) {
  ...
  for (int i = 0; true; ++i) {
    ...
    if (!status.ok()) {
      ...
      ASSERT_THAT(
          std::string(std::string(status.message())),
  ...
  ASSERT_THAT(std::string(std::string(status.message())),
              testing::HasSubstr("Meant to fail (magicstringincludedhere)."));
}
...
TEST_F(CalculatorGraphEventLoopTest, SetStreamHeader) {
  ...
  EXPECT_THAT(std::string(std::string(status.message())),
...
TEST_F(CalculatorGraphEventLoopTest, ThrottleGraphInputStreamTwice) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    EXPECT_THAT(std::string(std::string(status.message())), testing::HasSubstr("Graph is throttled."));
    MP_ASSERT_OK(graph.CloseInputStream("input_numbers"));
    MP_ASSERT_OK(graph.WaitUntilDone());
  }
}
