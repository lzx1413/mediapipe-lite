...
TEST(CalculatorGraph, OutputSidePacketAlreadySet) {
  ...
  EXPECT_THAT(std::string(status.message()), testing::HasSubstr("was already set."));
}
...
TEST(CalculatorGraph, OutputSidePacketWithTimestamp) {
  ...
  EXPECT_THAT(std::string(status.message()), testing::HasSubstr("has a timestamp 237."));
}
...
TEST(CalculatorGraph, SourceLayerInversion) {
  ...
  EXPECT_THAT(std::string(status.message()), testing::HasSubstr("deadlock"));
}
